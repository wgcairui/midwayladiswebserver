...
export class APIController {
  ...
  async validationSmsCode(
    @Body() code: string,
    @Body() openUser: Uart.WX.webUserInfo,
    @Body() tel: string,
    @Body() company: string
  ) {
    ...
    if (
      agent &&
      agent.contactTel.some(el => parseInt(el) === parseInt(tel)) &&
      codes &&
      codes === code
    ) {
  ...
  async getAgents(@Body() user: Uart.UserInfo) {
    const data = await this.Agent.getAgents(
      user.userGroup === 'admin' ? {} : { name: user.company }
    );
    return {
      code: 200,
      data,
