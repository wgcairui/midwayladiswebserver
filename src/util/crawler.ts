...
import { caseList, cases, DbTables, GMpack, pageLink, product, productList, support, supportAsid, supportList, supportProblem } from "../../types/typeing";
...
export class Crawler {
    ...
    async loadUrl(url: string, opt?: { title: string, parent?: string }) {
        const data = await axios.get('http://www.ladis.com.cn' + url, { timeout: 10000 })
            .then(res => {
                return {
                    code: 200,
                    data: res.data
    ...
    meta() {
        ...
        this.$("meta").each((i, el) => {
            if (el.attribs.name === 'keywords') this.data.Pagekeywords = el.attribs.content
            if (el.attribs.name === 'description') this.data.Pagedescription = el.attribs.content
        })
    }
    ...
    async support_down() {
        ...
        for (let i of e) {
            if (i.href.includes(".shtml")) {
                const d = await this.loadUrl(i.href, { title: this.data.MainTitle, parent: this.data.MainParent })
                let info: support = {
                    ...d.data,
                    link: i.href,
                    type: "soft",
                ...
                let down: support = { ...this.data, type: "pdf", ...i };
                data.push(down);
            }
        }
        return data;
