...
export class ContentCaseNewCache {
  ...
  async GetContentCtx(type: string, link: string) {
    ...
    const ids = [
      new Types.ObjectId(cacheData?.pre || doc._id),
      new Types.ObjectId(cacheData?.next || doc._id),
    ];
  ...
  async seachProducts(str: string) {
    ...
    return await model
      .find({ $or: [{ Pagekeywords: regstr }, { title: regstr }] })
      .lean();
  }
  ...
  async getDoment() {
    ...
    if (table === 'News' || table === 'Case') {
      const model =
        table === 'Case' ? getModelForClass(Case) : getModelForClass(New);
      if (isNews) {
        const list = await model
          .find({ company: SiteName })
          .sort({ 'data.time': -1 })
          .lean();
        if (list.length === 0)
    ...
    const model = getModelForClass(this.dbs[table], {
